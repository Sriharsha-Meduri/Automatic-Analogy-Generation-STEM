...
\title{\bfseries Automatic Analogy Generation for STEM Education:\\
Prompting Strategies and the Limits of Automated Quality Evaluation}

\author{\textbf{Sriharsha Meduri} \quad Mohit Pratap Singh Rathore \quad Gunveer Kalsi \\[3pt]
  \small Oviqo \\[2pt]
...
planted analogies of known quality. Constraining the source domain confers no quality
advantage over unconstrained generation: equivalence tests place all four strategies
within a small margin, and the two protocols' mild orderings point in opposite
directions, which we trace to a verbosity bias. Judged quality is highly predictable
($R^2 = 0.72$) but mainly from length and subject, not strategy. The panel reliably
screens out planted-wrong analogies yet barely agrees when ranking good ones; a plain
explanation lifts weak students' accuracy while an added analogy does not; and a preliminary Hindi
replication is consistent with the same finding, while the automated evaluation itself does not
carry over.
A judge twice the panel's size sheds the verbosity bias entirely and resolves a small
significant advantage for unconstrained generation, so the equivalence is partly the small
panel's own limitation. The paper contributes a reusable, fully automated, reproducible
...
\begin{enumerate}[leftmargin=1.6em]
  \item A fully automated, reproducible protocol for generating STEM analogies
    under controlled prompting strategies and evaluating them with a panel of
    small open models on a single consumer GPU, with no paid API or human
    annotation in the loop.
  \item The STEM Analogy Quality Rubric (SAQR), a five-dimension rubric for
    instructional analogies, together with a \emph{judge-validation probe} of
    planted known-quality analogies that objectively tests whether an automated
    judge can tell good analogies from bad ones.
  \item Empirical findings on prompting strategy (no reliable source-domain
    effect), on what the automated judges actually reward (surface length and
    subject, a known judge bias that we quantify in this instructional setting),
    on how an added analogy interacts with a proxy student's capability, and on
    how far all of these carry into Hindi rather than only English.
  \item A candid account of two failure modes of automated evaluation (student
    ceiling effects, and lenient, position-biased judges) and the mitigations
    (weak-student gradients, comparative ranking, label shuffling, probe
    validation) that keep the protocol honest.
\end{enumerate}
...
Several limitations bound our claims, and we state them plainly because the paper's value
depends on being honest about what automated evaluation can and cannot do.

\emph{Proxies for people, with objective anchors.} The judges and the ``students'' are
language models standing in for human teachers and learners, and this is the paper's central
scope limitation: absent human ratings, our quality and comprehension numbers are automated
proxy measurements, and any claim about human learning would ultimately require human
validation, which we leave to future work. Two of our four measurements are, however,
deliberately \emph{not} one model judging another. The judge-validation probe scores each panel
against planted analogies of \emph{known}, author-fixed quality (including analogies that are
objectively wrong), so it measures discrimination against ground truth, not against a model's
opinion. And the comprehension study (RQ3) scores multiple-choice answers objectively against
fixed keys, so it is a behavioural outcome rather than a quality judgement. That scoring is
objective, but the learner is still a model: a language model can select the right option by
pattern-matching without anything resembling human understanding, so RQ3 is best read as measuring
whether a teaching text carries information a model can \emph{use}, and it bears on human learning
only indirectly. A null there does not prove an analogy useless to a student, nor would a gain
prove it teaches; establishing either would take human learners. These objective
anchors are what let us state the paper's firmest claims (the panel discriminates planted-wrong
analogies; a plain explanation raises weak-student accuracy) without resting them on one model
rating another. The remaining subjective judgements (the rubric panel) are reported throughout
with their agreement caveats.

\emph{Small-model judging.} Our judges are 2--3.8B-parameter models chosen to run on a 4\,GB
GPU. They are lenient, they disagree on near-equal items, and, before we shuffled labels and
switched to ranking, some defaulted to presentation order rather than reading the analogies.
We mitigate these behaviours and validate against a probe, but a stronger judge would likely
give finer, more reliable discrimination. Our larger-judge check (Section~\ref{sec:results:scale})
bears this out in part: at 7B the screening and no-effect findings replicate and the verbosity
bias weakens, but frontier-scale judges remain untested, so some quantitative details (the exact
agreement levels and the size of the verbosity bias) may be specific to this model scale.

\emph{Scope of the dataset.} We study 64 concepts across four subjects at grades 8--12, with a
16-concept multiple-choice subset. This is enough to detect subject-level patterns but is far
from full curriculum coverage, and the concepts, explanations, and questions were authored by
a single researcher, which may embed idiosyncratic choices despite our attempts at
curriculum fidelity. The parts that serve as \emph{ground truth}, however, are objective rather
than matters of authorial taste: the multiple-choice keys are verifiable curriculum facts and
the probe's planted-wrong analogies contain clear, checkable errors, so what the judges and
students are scored against does not rest on one person's quality opinions.

\emph{Cross-lingual scope.} The Hindi replication covers a single language, 20 concepts, and a
code-mixed register, and the low Hindi parse rates mean that some Hindi metrics (notably the
absolute-scoring comparisons, computed on the 35\% of outputs that parsed) rest on limited
data. It establishes that the automated protocol degrades sharply outside English, but not
precisely how it would behave for other languages or scripts. For the same reason the apparent
replication of the no-strategy-effect finding in Hindi is best read as exploratory: with the panel
barely able to tell good analogies from bad, we cannot cleanly separate a genuine absence of effect
from an effect the degraded tooling simply could not detect.

\emph{Generator/judge overlap and self-preference.} Three of the four panel models also serve
as generators. We exclude every model from judging its own analogies, which removes direct
self-preference, but shared training lineages could still induce subtler family-level
preferences that our design does not fully rule out.

\emph{Rubric validation.} SAQR is grounded in the learning-sciences literature and is
behaviourally validated only indirectly (through the discrimination probe and the
comprehension study). A formal validation against expert human ratings, and a test of its
dimensions' independence, remain future work.
